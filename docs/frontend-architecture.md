...
- **`pages/`** — Route-entry files and route-level composition. They should stay thin and prefer delegating detailed behavior to feature hooks and UI sections. For **in-folder page modules** (e.g. Employee Home), follow **[page architecture contract](frontend/page-architecture-contract.md)** (`services/`, `adapters/`, `state/`, `hooks/`, presentational shell) and the **[frontend project blueprint](architecture/frontend-project-blueprint.md)** for repo-wide layout, tests, and documentation.
