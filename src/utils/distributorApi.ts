...
export async function distributorsSummary(token: string, range: DateRange) {
  ...
  const res = await fetch(`http://localhost:5000/api/distributors/summary?${qs.toString()}`, {
    headers: authHeaders(token),
  });
...
export async function createDistributor(token: string, payload: { name: string; commissionPct?: number }) {
  const res = await fetch("http://localhost:5000/api/distributors", {
    method: "POST",
    headers: authHeaders(token),
...
export async function updateDistributor(token: string, id: string, payload: Partial<{ name: string; commissionPct: number }>) {
...
export async function distributorCustomers(token: string, id: string) {
  const res = await fetch(`http://localhost:5000/api/distributors/${id}/customers`, {
    headers: authHeaders(token),
  });
...
export async function distributorSummary(token: string, id: string, range: DateRange) {
  const qs = new URLSearchParams();
  if (range.from) qs.set("from", range.from);
  if (range.to) qs.set("to", range.to);
...
export async function assignCustomerToDistributor(token: string, customerId: string, distributorId: string) {
  const res = await fetch(`http://localhost:5000/api/distributors/customers/${customerId}/assign`, {
    method: "PATCH",
    headers: authHeaders(token),
...
export async function unassignCustomerFromDistributor(token: string, customerId: string) {
  const res = await fetch(`http://localhost:5000/api/distributors/customers/${customerId}/unassign`, {
    method: "PATCH",
    headers: authHeaders(token),
  });
  const data = await res.json().catch(() => ({}));
