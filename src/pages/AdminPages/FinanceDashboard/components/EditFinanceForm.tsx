// src/pages/AdminPages/FinanceDashboard/components/EditFinanceForm.tsx
import React, { useState, useEffect } from "react";
import { Category, FinanceEntry, Payment } from "../../../../features/finance/types";
import { catAr } from "../../../../features/finance/utils/financeUtils";
import { isNewFinanceShape } from "../../../../features/finance/utils/financeUtils";
...
export default function EditFinanceForm({
  entry,
  cats,
  onSave,
  onCancel,
  isSubmitting = false,
}: EditFinanceFormProps) {
  const [form, setForm] = useState({
    kind: entry.kind || "expense" as "income" | "expense",
  ...
  // Initialize payments from entry
  const [payments, setPayments] = useState<PaymentRow[]>(() => {
    if (isNewFinanceShape(entry) && entry.payments && entry.payments.length > 0) {
      return entry.payments.map((p: Payment) => ({
        date: p.date ? new Date(p.date).toISOString().slice(0, 10) : form.date,
        amount: p.amount,
        currency: p.currency,
        paymentMethod: p.paymentMethod || "",
        note: p.note || "",
        rateAtPaymentLBP: p.rateAtPaymentLBP,
      }));
    }
    // Legacy shape
    return [{
      amount: entry.amount || "",
      currency: (entry.currency as "USD" | "LBP") || "USD",
      date: form.date,
    }];
  });

  const addPaymentRow = () =>
    setPayments((rows) => [...rows, { amount: "", currency: "USD", date: form.date }]);
