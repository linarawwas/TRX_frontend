// actionTypes.ts
// Action types for setting token and company ID
export const SET_CUSTOMER_ID = "SET_CUSTOMER_ID";
export const SET_AREA_ID = "SET_AREA_ID";
export const SET_PRODUCT_ID = "SET_PRODUCT_ID";
export const SET_PRODUCT_NAME = "SET_PRODUCT_NAME";
export const SET_PRODUCT_PRICE = "SET_PRODUCT_PRICE";
export const SET_CUSTOMER_NAME = "SET_CUSTOMER_NAME";
export const SET_CUSTOMER_PHONE_NB = "SET_CUSTOMER_PHONE_NB";
// Action types for clearing token and company ID
export const CLEAR_CUSTOMER_ID = "CLEAR_CUSTOMER_ID";
export const CLEAR_AREA_ID = "CLEAR_AREA_ID";
export const CLEAR_PRODUCT_ID = "CLEAR_PRODUCT_ID";
export const CLEAR_PRODUCT_NAME = "CLEAR_PRODUCT_NAME";
export const CLEAR_PRODUCT_PRICE = "CLEAR_PRODUCT_PRICE";
export const CLEAR_CUSTOMER_NAME = "CLEAR_CUSTOMER_NAME";
export const CLEAR_CUSTOMER_PHONE_NB = "CLEAR_CUSTOMER_PHONE_NB";

