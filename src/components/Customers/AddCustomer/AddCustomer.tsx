...
const AddCustomer: React.FC = () => {
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await fetch("https://trx-api.theagilelabs.com/api/areas/company", {
    ...
    (async () => {
      try {
        const res = await fetch(
          `https://trx-api.theagilelabs.com/api/customers/area/${currentAreaId}/active`,
          { headers: { Authorization: `Bearer ${token}` } }
        );
        if (!res.ok) throw new Error();
  ...
  const onSubmit = async (data: Record<string, any>) => {
    ...
    const res = await fetch(
      "https://trx-api.theagilelabs.com/api/customers/create-with-sequence",
      {
        method: "POST",
        headers: {
