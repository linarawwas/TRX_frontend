...
- **Sends a `POST` request** to `https://api-trx.linarawas.com/api/areas/days/${dayId}` with `companyId` in the request body.
...
- A separate `useEffect` fetches the day name using `GET https://api-trx.linarawas.com/api/days/${dayId}`.
