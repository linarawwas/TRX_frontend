...
const ExtraProfits: React.FC = () => {
    ...
    useEffect(() => {
        const fetchExtraProfits = async () => {
            try {
                const response = await axios.get(`https://api-trx.linarawas.com/api/extraProfits/company/${companyId}`, {
                    headers: {
    ...
    const handleDeleteProfit = async (profitId: string) => {
        try {
            const response = await fetch(`https://api-trx.linarawas.com/api/extraProfits/${profitId}`, {
                method: 'DELETE',
                headers: {
                    Authorization: `Bearer ${token}`,
                },
