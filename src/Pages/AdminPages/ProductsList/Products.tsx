...
const ProductsList: React.FC = () => {
    ...
    useEffect(() => {
        const fetchProducts = async () => {
            try {
                const response = await axios.get(`https://api-trx.linarawas.com/api/products/company/${companyId}`, {
                    headers: {
    ...
    const handleDeleteExpense = async (productId: string) => {
        try {
            const response = await fetch(`https://api-trx.linarawas.com/api/products/${productId}`, {
                method: 'DELETE',
                headers: {
                    Authorization: `Bearer ${token}`,
                },
